...
- "Newer than installed" does not use pacman's `vercmp`. The Agent CLI and Origin publish a build hash after the release date (`2026.09.08-6caf4ff`, `2026.09.08-22-50-39-8f6b2f8`), and `vercmp` reads everything after the last hyphen as a pkgrel, so it orders same-day builds by their commit hash and calls a fresh one older. `vendor_version_newer` splits both versions on `.` and `-` and walks the fields instead: the first differing pair decides, numbers compare numerically, and a pair that is not a number on both sides is build identity, so a difference there means the vendor published a different build at the same release position and the vendor's build wins. An older release date still never downgrades.
