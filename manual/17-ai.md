...
Omarchy treats AI coding agents as first-class citizens, but it doesn't pick a favorite for you. Most coding-agent CLIs come pre-wired as lazy-loaded [mise](https://mise.jdx.dev/) stubs in `~/.local/bin/`, so nothing is downloaded until the first time you actually run one. Cursor Agent is one exception: picking it as the default uses a `cursor-agent` already on PATH, or installs Cursor's official Linux tarball as `cursor-agent` (not `agent`). After that, `omarchy update` and `omarchy update cursor-agent` install a newer official tarball when one is published. It is not install-once. Hermes is the other: nothing is stubbed for it, and choosing it installs Hermes Desktop, as _Desktop apps_ below explains. Invoke any of these and authenticate when prompted:
...
| `cursor-agent` | [Cursor Agent](https://cursor.com/docs/cli/overview) from Cursor's official Linux tarball |
